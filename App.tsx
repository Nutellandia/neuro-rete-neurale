...
const App: React.FC = () => {
  ...
  const [isExporting, setIsExporting] = useState(false); // NEW STATE FOR FEEDBACK
  ...
  const maxPhysicsNodes = useMemo(() => {
    ...
    if (hardwareConcurrency >= 8 && !isMobile) return 5000; 
    if (hardwareConcurrency >= 6) return 2500; 
    return 1000; 
  }, []);
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => { 
        ...
        try {
            ...
            if (file.name.endsWith('.zip')) {
                ...
                // Try reading standard location first (neural_data folder), then root
                const brainFile = Object.keys(contents.files).find(name => name.includes('brain_dna.json'));
                if (brainFile) {
                    const text = await contents.files[brainFile].async("string");
                    jsonData = JSON.parse(text);
                } else { throw new Error("No Brain DNA found in ZIP"); }
            } else {
  ...
  // VERIFICA PASSWORD E SCARICA
  const handleAuthAndDownload = async () => {
      if (authInput === "FRASCOGNA-ARCH") {
          setIsAuthModalOpen(true); // Tieni il modale aperto per feedback
          setAuthError(false);
          setIsExporting(true); // Attiva spinner
          
          // CRITICO: Usa setTimeout per permettere a React di renderizzare lo stato "Caricamento"
          // prima che l'operazione pesante (JSZip) blocchi il thread principale.
          setTimeout(async () => {
              try {
                  await exportProjectToZip(brain, logs);
                  addLog("SYSTEM", "FULL EXPORT (SOURCE+DATA) AUTHORIZED.");
                  setIsAuthModalOpen(false); // Chiudi solo a successo avvenuto
              } catch (e) {
                  console.error(e);
  ...
  return (
    ...
    >
       {/* AUTH MODAL */}
       {isAuthModalOpen && (
           <div className="absolute inset-0 z-[150] bg-black/95 flex items-center justify-center p-4 backdrop-blur-md">
               <div className="bg-[#111] border border-cyan-900/50 p-6 rounded-lg max-w-sm w-full text-center shadow-2xl animate-fade-in relative">
                   <button onClick={() => setIsAuthModalOpen(false)} className="absolute top-2 right-2 text-gray-500 hover:text-white">✕</button>
                   <div className="text-4xl mb-4">🔐</div>
                   <h2 className="text-lg font-mono text-cyan-400 font-bold mb-2">IDENTIFICAZIONE ARCHITETTO</h2>
                   <p className="text-xs text-gray-400 mb-4">Inserisci la chiave crittografica per decifrare ed esportare il codice sorgente originale.</p>
                   <input 
                       type="password" 
                       value={authInput}
                       onChange={(e) => setAuthInput(e.target.value)}
                       disabled={isExporting}
                       className="w-full bg-black border border-gray-700 p-2 rounded text-center text-cyan-400 font-mono focus:border-cyan-500 outline-none mb-4 disabled:opacity-50"
                       placeholder="PASSWORD"
                   />
                   {authError && <div className="text-red-500 text-xs font-bold mb-2">ACCESSO NEGATO.</div>}
                   <button 
                        onClick={handleAuthAndDownload} 
                        disabled={isExporting}
                        className={`w-full py-2 border rounded font-mono uppercase font-bold transition-all ${isExporting ? 'bg-yellow-900/50 text-yellow-200 border-yellow-700 cursor-wait' : 'bg-cyan-900/50 hover:bg-cyan-800 text-cyan-200 border-cyan-700'}`}
                    >
                       {isExporting ? "GENERAZIONE ZIP..." : "SBLOCCA & SCARICA"}
                   </button>
               </div>
           </div>
       )}
                ...
                <div className="bg-[#111] border border-cyan-900/50 p-6 rounded-lg max-w-lg w-full shadow-2xl relative flex flex-col max-h-[90vh]">
                    ...
                    <div className="overflow-y-auto space-y-4 pr-2 font-mono text-xs text-gray-300 scrollbar-thin scrollbar-thumb-gray-800 leading-relaxed">
                        ...
                        <div className="text-[9px] text-gray-500 mt-2 italic text-center">
                            Architettura E. Frascogna - v5.1 (Data Backup Only)
                        </div>
                    </div>
       ...
       {isPaused && !isDead && !isAuthModalOpen && (
                    ...
                    <div className="p-6 overflow-y-auto space-y-6 text-gray-300 font-mono text-xs flex-1">
                        ...
                        <div className="bg-gray-900 p-4 rounded border border-gray-700 mt-4">
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <button onClick={() => setIsAuthModalOpen(true)} className="py-3 px-4 bg-cyan-900/30 hover:bg-cyan-800 border border-cyan-600 rounded text-cyan-200 uppercase font-bold flex items-center justify-center gap-2 transition-all w-full"><span>💾</span> SCARICA SORGENTE ORIGINALE (.ZIP)</button>
                            </div>
                        </div>
                    </div>
               </div>
       ...
       <div className="p-3 landscape:p-1 border-b border-gray-800 bg-[#0f1219] flex justify-between items-center shrink-0 z-[60] relative shadow-md">
            ...
            <div className="flex gap-2">
                    ...
                    <button onClick={() => setIsAuthModalOpen(true)} className="p-2 text-cyan-600 hover:text-cyan-300 hover:bg-gray-800 rounded transition-colors" title="Scarica Backup Completo (.zip)">💾</button>
